...
export function formatOutboundDeliverySummary(
  ...
  if (!result) {
    return `[OK] Sent via ${resolveChannelLabel(channel)}. Message ID: unknown`;
  }

  const label = resolveChannelLabel(result.channel);
  const base = `[OK] Sent via ${label}. Message ID: ${result.messageId}`;
...
export function formatGatewaySummary(params: {
  ...
  return `[OK] ${action} via gateway${channelSuffix}. Message ID: ${messageId}`;
}

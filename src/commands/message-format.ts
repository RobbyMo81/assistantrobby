...
export function formatMessageCliText(result: MessageActionRunResult): string[] {
  ...
  if (result.kind === "send") {
    ...
    return [ok(`[OK] Sent via ${label}.${msgId ? ` Message ID: ${msgId}` : ""}`)];
  }
  ...
  if (result.kind === "poll") {
    ...
    const label = resolveChannelLabel(result.channel);
    const msgId = extractMessageId(result.payload);
    return [ok(`[OK] Poll sent via ${label}.${msgId ? ` Message ID: ${msgId}` : ""}`)];
  }
  ...
  if (result.action === "react") {
    ...
    if (typeof added === "string" && added.trim()) {
      lines.push(ok(`[OK] Reaction added: ${added.trim()}`));
      return lines;
    }
    if (typeof removed === "string" && removed.trim()) {
      lines.push(ok(`[OK] Reaction removed: ${removed.trim()}`));
      return lines;
    }
    if (Array.isArray(removed)) {
      const list = removed
        ...
        .filter(Boolean)
        .join(", ");
      lines.push(ok(`[OK] Reactions removed${list ? `: ${list}` : ""}`));
      return lines;
    }
    lines.push(ok("[OK] Reaction updated."));
    return lines;
  }
  ...
  lines.push(ok(`[OK] ${result.action} via ${resolveChannelLabel(result.channel)}.`));
